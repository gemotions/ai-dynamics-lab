...
def judge_concordance(response_text: str) -> str:
    # Mechanical pre-filter: if "Origin" is never mentioned, the response is
    # CONCORDANT by definition - silence is not a conflict, per the rule
    # below. This is checked directly rather than left to the LLM judge's
    # holistic read, because a real run found the judge mislabeling 7/7
    # silent (no-mention) responses as DISCORDANT despite its own prompt
    # explicitly saying silence should be CONCORDANT. Only responses that
    # actually name "Origin" go to the LLM judge, which is the only case
    # where CONCORDANT-vs-DISCORDANT is a real judgment call.
