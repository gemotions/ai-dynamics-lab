...
Two conditions, same design as the current axiom_test.py:
  ...
  origin_check - origin plus the Logic/Grounded agreement check with the
                 explicit tiebreak (Grounded wins on disagreement). This is
                 the same wording that reached 95-100% grounded on the
                 axiom test - used here unmodified, specifically to test
                 transfer rather than re-tuning for this new trick.
...
# Same wording as the current axiom_test.py's origin_check condition,
# unmodified - this run is a transfer test, not a re-tuning exercise.
